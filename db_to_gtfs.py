...
class DBApiToGTFS(object):
    ...
    def get_all_trips_for_stop(self, stop):
        ...
        while stop['last_date'] < self.end_date:
            logging.info('@ #%d (%s) on %s %s, unique trips collected: %d, unprocessed stations left: %d', stop['stop_id'], stop['stop_name'].encode('utf-8').strip(), stop['last_date'].strftime('%Y-%m-%d'), str(stop['last_check_h']) + ':' + str(stop['last_check_m']), len(self.trips), self.unproced_counter)
